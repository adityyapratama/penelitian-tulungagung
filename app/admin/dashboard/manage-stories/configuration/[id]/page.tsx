...
import { Play, PlayIcon, Plus, Save, Upload } from "lucide-react";
...
export default function Page() {
      ...
      if (deletedEdges.length > 0) {
        setScenes((currentScenes) => {
          ...
          deletedEdges.forEach((change) => {
            const deletedEdge = edgesSnapshot.find(
              (edge) => edge.id === change.id
            );
            if (deletedEdge) {
              const sourceSceneIndex = updatedScenes.findIndex(
                (scene) => scene.scene_key === deletedEdge.source
              );
          ...
          setNodes(createNodesFromScenes(updatedScenes));
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    setScenes((currentScenes) => {
      const updatedScenes = [...currentScenes];
      const sourceSceneIndex = updatedScenes.findIndex(
        (scene) => scene.scene_key === params.source
      );
      const targetScene = updatedScenes.find(
        (scene) => scene.scene_key === params.target
      );

      if (sourceSceneIndex !== -1 && targetScene) {
        const sourceScene = updatedScenes[sourceSceneIndex];
        const existingChoice = sourceScene.scene_choices.find(
          (choice) => choice.next === params.target
        );
        if (!existingChoice) {
  ...
  const updateScene = useCallback(
    (updatedScene: StoryScene) => {
      const originalScene = scenes.find(
        (scene) => scene.scene_key === selectedScene?.scene_key
      );
      const oldSceneKey = originalScene?.scene_key;
      const newSceneKey = updatedScene.scene_key;

      let updatedScenes = scenes.map((scene) =>
        scene.scene_key === oldSceneKey ? updatedScene : scene
      );

      // If scene_key was changed, update all scene_choices that reference the old key
      if (oldSceneKey && newSceneKey && oldSceneKey !== newSceneKey) {
        updatedScenes = updatedScenes.map((scene) => ({
          ...
          scene_choices: scene.scene_choices.map((choice) =>
            choice.next === oldSceneKey
              ? { ...choice, next: newSceneKey }
              : choice
          ),
        }));
      }
  ...
  const deleteScene = useCallback(
    (sceneKey: string) => {
      const updatedScenes = scenes.filter(
        (scene) => scene.scene_key !== sceneKey
      );
      setScenes(updatedScenes);
      setNodes(createNodesFromScenes(updatedScenes));
      setEdges(createEdgesFromScenes(updatedScenes));
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success && result.scenes) {
          const convertedScenes: StoryScene[] = result.scenes.map(
            (scene: any) => ({
              scene_key: scene.scene_key,
              scene_text: scene.scene_text,
              scene_choices: scene.scene_choices || [],
  ...
  return (
    <div className="w-screen h-screen relative">
      <div className="absolute top-4 left-4 z-10 flex gap-2">
        ...
        <Button
          onClick={() =>
            redirect(`/admin/dashboard/manage-stories/demo/${storyId}`)
          }
          disabled={isPending}
